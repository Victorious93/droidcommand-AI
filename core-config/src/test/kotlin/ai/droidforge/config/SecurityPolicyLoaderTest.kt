...
class SecurityPolicyLoaderTest {
    ...
    @Test
    fun `parses DROIDFORGE_ROOT_ENABLED`() {
        val policy = SecurityPolicyLoader.load(MapConfigSource(mapOf("DROIDFORGE_ROOT_ENABLED" to "true")))
    ...
    fun `parses a comma-separated, trimmed permission list`() {
        val policy = SecurityPolicyLoader.load(
            MapConfigSource(mapOf("DROIDFORGE_GRANTED_PERMISSIONS" to "CAMERA, MICROPHONE ,STORAGE")),
        )
    ...
    fun `parses a comma-separated, case-insensitive auto-approve level list`() {
        val policy = SecurityPolicyLoader.load(
            MapConfigSource(mapOf("DROIDFORGE_AUTO_APPROVE_LEVELS" to "normal, sensitive")),
        )
    ...
    fun `an unrecognized security level name fails fast instead of being silently ignored`() {
        assertFailsWith<IllegalArgumentException> {
            SecurityPolicyLoader.load(MapConfigSource(mapOf("DROIDFORGE_AUTO_APPROVE_LEVELS" to "not-a-real-level")))
        }
    }
