...
- `core-remote` — the first module with a genuinely working implementation,
  ...
  200). `JdkHttpTransport` also takes an optional `CertificatePinner`,
  which pins a specific certificate's public key instead of trusting
  whatever a CA vouches for — tested against a real TLS handshake with a
  real self-signed certificate generated by the JDK's own `keytool`: the
  correct pin lets the handshake complete, a wrong one fails it for real,
  and with no pinner the same certificate is correctly rejected by
  ordinary CA trust. Still PLANNED: mutual TLS (client certificates), and
  a build-server client actually built on top of this transport —
