...
- `core-remote` — the first module with a genuinely working implementation,
  ...
  200). `JdkHttpTransport` also takes an optional `MutualTlsConfig` for
  mutual TLS — presenting a client certificate, trusting the server
  against a private CA, or both — tested against a real TLS handshake
  with a real private CA and server/client certificate chain generated by
  the JDK's own `keytool`: the correct client certificate lets a real
  ...
  rejected by ordinary platform CA trust. Still PLANNED: certificate
  pinning (a separate, narrower trust mechanism than mTLS), and a
