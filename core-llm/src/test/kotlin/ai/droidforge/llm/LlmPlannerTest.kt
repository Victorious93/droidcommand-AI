...
class LlmPlannerTest {
    ...
    fun `translates a tool-call response into InvokeTool`() {
        ...
        val invoke = assertIs<ai.droidforge.agent.PlannerDecision.InvokeTool>(decision)
    ...
    fun `translates a text response into Complete`() {
        ...
        val complete = assertIs<ai.droidforge.agent.PlannerDecision.Complete>(decision)
    ...
    fun `translates a provider error into Abort instead of throwing`() {
        ...
        val abort = assertIs<ai.droidforge.agent.PlannerDecision.Abort>(decision)
    ...
    fun `forwards the conversation and system prompt into the request`() {
        ...
        val context = ConversationContext(systemPrompt = "You are DroidForge AI").apply {
        ...
        assertEquals("You are DroidForge AI", request.systemPrompt)
