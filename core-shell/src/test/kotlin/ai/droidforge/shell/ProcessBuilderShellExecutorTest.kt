...
class ProcessBuilderShellExecutorTest {
    ...
    fun `runs in an authorized working directory for real`() {
        val tempDir = Files.createTempDirectory("droidforge-shell-test").toFile()
        try {
    ...
    fun `denies a working directory outside the authorized root, even a real existing one`() {
        val authorizedDir = Files.createTempDirectory("droidforge-shell-authorized").toFile()
        val outsideDir = Files.createTempDirectory("droidforge-shell-outside").toFile()
        try {
            val executor = ProcessBuilderShellExecutor(
    ...
    fun `passes extra environment variables through to the real process`() {
        ...
        val result = assertIs<ShellExecutionResult.Success>(
            executor.execute(ShellCommand(executable = "env", environment = mapOf("DROIDFORGE_TEST_VAR" to "test-value-123"))),
        )
        assertTrue(result.stdout.contains("DROIDFORGE_TEST_VAR=test-value-123"))
    }
