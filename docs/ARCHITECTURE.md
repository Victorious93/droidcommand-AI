...
| core-agent: CapabilityId / ExecutionRequest / ExecutionResponse / RiskTier / ExecutionTargetType / CapabilityRegistry / CapabilityExecutor / ExecutionRouter / DefaultExecutionRouter (CAP-008, first slice) | IMPLEMENTED (domain contract + dispatch logic; no real capability registry or execution target backs it yet) | Added 2026-09-11 — `ExecutionRouting.kt`, closing P1's first item and the gap the CAP-### reconciliation named for CAP-008: "Current contract is `ToolSpec`/`ToolResult` + `PolicyDecision` — a materially different, narrower shape. No `CapabilityId`/`RiskTier`/`ExecutionTargetType` type exists." Unlike P0.7, P1.0 hands over literal, complete Kotlin types, shipped verbatim: `CapabilityId` (regex-validated at construction, throwing `IllegalArgumentException` immediately on an invalid value — the same "make the illegal state unrepresentable" discipline `JsonFileKnowledgeStore`'s id validation already applies), `ExecutionRequest`, `ExecutionResponse` (4-case sealed class: `Success`/`RequiresApproval`/`Denied`/`CapabilityUnavailable`), `RiskTier` (4 values), `ExecutionTargetType` (7 values, none backed by a real target yet — see CAP-011/019/024/029, all MISSING). **Deliberately not unified with the existing `SecurityLevel`/`ToolSpec`/`PolicyDecision` contract** — that reconciliation is CAP-010's job (Policy & Permission Engine extension), a separate later roadmap item; the two systems stay parallel in this slice. Ships a real dispatch layer on top of the types, not types alone: `CapabilityRegistry` (`isRegistered(id): Boolean`, a minimal boolean gate — deliberately **not** CAP-009's full 11-state re-verifying registry, which needs real capabilities to register against), `CapabilityExecutor` (the seam a real `ExecutionTarget`-backed executor plugs into once CAP-011 exists — only a scripted fake exercises it in tests today), `ExecutionRouter`/`DefaultExecutionRouter`. `DefaultExecutionRouter.route` runs two gates before `executor` is ever consulted: (1) `registry.isRegistered` — an unregistered capability returns `CapabilityUnavailable` and the executor is never called, the concrete enforcement of P1.0's own prose requirement ("the agent must never invoke a capability string not present in the live registry"); (2) `riskTier !in autoApprove` (default `{READ_ONLY, REVERSIBLE}`, drawn directly from the roadmap prompt's own inline enum comments — only `DESTRUCTIVE`/`IRREVERSIBLE` are annotated "requires approval") returns `RequiresApproval` immediately, non-blocking and carrying a generated `requestId`. **Deliberate deviation from `SecureToolExecutor`'s precedent:** `RequiresApproval` is a terminal response, not a synchronous blocking prompt callback like `ApprovalPrompt.requestApproval` — the roadmap prompt's own `RequiresApproval(operationDescription, riskTier, requestId)` shape is async by design; resolving/replaying a `requestId` into a later decision is a separate, unspecified mechanism, named as a real gap rather than invented. Only once both gates pass does `executor.execute` run, its `ExecutionResponse` returned unchanged — a real executor's own `Denied`/`CapabilityUnavailable` is never second-guessed. No new Gradle module (spec's `core-capability` folded into `core-agent`, the same call already made for CAP-004/005/006/007's own suggested modules); no wiring into `ObjectiveEngine`/`DroidCommandSession`/`ToolExecutor` (caller-opt-in, matching every prior CAP slice). Unit-tested (`CapabilityIdTest`, 6 cases; `DefaultExecutionRouterTest`, 12 cases): valid/invalid id patterns: unregistered-capability short-circuit (executor never invoked, proven via a recording fake), `READ_ONLY`/`REVERSIBLE` delegating by default, `DESTRUCTIVE`/`IRREVERSIBLE` requiring approval by default, a custom `autoApprove` set widening approval, a custom `requestIdGenerator` honored, the exact request reaching the executor unchanged, one log event per path at the expected level, and a real executor's own `Denied` response passed through unchanged. |
| core-agent: ExecutionTarget / ExecutionContext / PrivilegeLevel / ExecutionResult (CAP-011, first slice) | IMPLEMENTED (domain contract; one real target) | Added 2026-09-11 — `ExecutionTarget.kt`, closing P1.3's gap: "Refactor the real `ProcessBuilderShellExecutor` behind an abstraction that other targets can implement." Types shipped verbatim from the spec (`suspend` dropped from every method — no module in this repository declares a coroutines dependency, the same deviation `AiProviderSelector`/`PersonaManager` already state for themselves). `ExecutionTargetType`/`CapabilityId` are reused unchanged from CAP-008's `ExecutionRouting.kt`, not redefined. **A deliberate, stated name collision:** `ai.droidcommand.agent.PrivilegeLevel` (`USER`/`ELEVATED`/`ROOT`) coexists with the pre-existing, differently-shaped `ai.droidcommand.root.PrivilegeLevel` (`NONE`/`USER`/`ROOT`) — different packages, no compile-time collision, but a real, open duplication; reconciling the two privilege vocabularies is out of scope for this slice, the same "not unified here" call CAP-008 already made keeping `RiskTier` separate from `SecurityLevel`. `core-root.RootProvider`/`RootExecutor` (a specialized, already-wired-into-`core-security` root-command abstraction) is deliberately not touched or reused as a backing implementation either — unifying it with `ExecutionTarget` is real future work, not this slice's. `ExecutionResult` is a flat data class (unlike `ExecutionResponse`'s/`ShellExecutionResult`'s sealed shapes): failure is represented through `exitCode`/`stderr`, not a separate case; `verified` is never set `true` by anything in this repository yet — real result verification is CAP-012's `verifyResult`, a separate later item. |
| core-shell: ShellExecutorExecutionTarget (CAP-011, real implementation) | IMPLEMENTED | Added 2026-09-11 — `ShellExecutorExecutionTarget.kt`, adapting the real `ProcessBuilderShellExecutor`/`ShellExecutor` to `core-agent.ExecutionTarget`. **The concrete reading this repository takes of P1.3's "refactor... behind an abstraction": composition, not an in-place rewrite** — `ProcessBuilderShellExecutor`/`ShellExecutor` already has real, tested callers (`ShellTool`, `ShellToolSecureExecutorIntegrationTest`) a literal rewrite to `ExecutionTarget`'s different shape (`argv: List<String>` vs `ShellCommand`; a flat `ExecutionResult` vs the sealed `ShellExecutionResult`) would break for zero benefit — the same "adapter over existing tested code" choice `core-tools-android.DeviceContextProvider` already makes composing `DeviceController`. `type` is always `ExecutionTargetType.LOCAL_PC` (the correct mapping among CAP-008's seven target types for a local-JVM-host process); `isHealthy()` is unconditionally `true` (no external dependency to verify beyond the JVM itself being alive, unlike a future TERMUX/DOCKER/REMOTE_HOST target). `execute` maps `argv`'s head/tail to `ShellCommand.executable`/`args`; an empty `argv` is rejected before `shellExecutor` is ever called (fail before touching anything, matching `ProcessBuilderShellExecutor`'s own policy-check discipline); per-call `workingDir`/`env` override `context`'s own defaults when given (`?:` fallback), matching the spec's own nullable-override signature shape. **A real, honestly-stated interaction between two independently fail-closed designs:** since `ExecutionContext.workingDir` is non-nullable, `execute` always passes a non-null `ShellCommand.workingDirectory`, so `ShellSecurityPolicy.allowedWorkingDirectories` (empty by default) must explicitly allow-list it or every call fails closed with a policy rejection — correct behavior, not a bug, but easy to miss and so stated directly in the class's own doc comment. `ShellExecutionResult.Failure` maps to `exitCode = -1`/`stderr = reason`; **a stated, honest limitation:** `ShellExecutionResult` carries no structured timeout signal today (only a human-readable `Failure.reason` string), so `ExecutionResult.timedOut` is always `false` on that path rather than guessed via fragile string-matching; `verified` is always `false` (no real verification in this slice). Unit-tested (`ShellExecutorExecutionTargetTest`, 9 cases): a real end-to-end `echo` run through the actual `ProcessBuilderShellExecutor` (not mocked, matching `ProcessBuilderShellExecutorTest`'s own convention, with both the executable and its working directory correctly allow-listed), plus a recording fake `ShellExecutor` proving empty-`argv` short-circuit, argv-to-executable/args mapping, per-call `workingDir`/`env` override vs. context-default fallback, `timeoutMs` forwarding, `Failure` mapping, unconditional `isHealthy()`, and `id`/`type`/`context`/`availableCapabilities` exposure. |
| core-security: RoutingDecision / ExecutionTargetRouter / DefaultExecutionTargetRouter (CAP-012, first slice) | IMPLEMENTED | Added 2026-09-11 — `ExecutionTargetRouting.kt`, closing P1.4 and completing this session's own device-free-verifiable slice of P1 (CAP-008/011/012). **The naming collision the CAP-011 entry flagged is resolved here:** the roadmap prompt names *two different* interfaces `ExecutionRouter` in its own P1.0 and P1.4 sections — CAP-008's `core-agent.ExecutionRouter` (routes one request to a single `CapabilityExecutor` by `CapabilityId`/`RiskTier`) is untouched; this new one is named `ExecutionTargetRouter` and lives in `core-security` (not `core-agent`), since its own literal signature takes a `SecurityPolicyEnforcer` parameter — `core-security` already depends on `core-agent` one-way for exactly this reason (`SecureToolExecutor` does the same). `RoutingDecision` (`Route`/`NoSuitableTarget`) is shipped verbatim. Two deviations, same reasoning used throughout this session: `suspend` dropped from both interface methods (no coroutines dependency anywhere in this repository); `routeExecution` returns a non-null `RoutingDecision`, never the spec's nullable one, since a bare `null` would discard exactly the "why" `NoSuitableTarget(reason)` exists to carry. `DefaultExecutionTargetRouter(toolRegistry: ToolRegistry)` follows the roadmap's own 4-step router logic in order: (1) filter `availableTargets` to those declaring the request's `CapabilityId`, short-circuiting to `NoSuitableTarget` without ever resolving `toolId` or consulting the enforcer if none qualify; (2) resolve `toolId` via `toolRegistry.get` (an unregistered tool → `NoSuitableTarget`) and call `securityEnforcer.authorize` (a `Deny` → `NoSuitableTarget`, even with a capable target available); (3) pick the least-privileged capable target — ascending `PrivilegeLevel.ordinal`, tie-broken by `id` for determinism; (4) return `Route(target, decision)` with whichever `Allow`/`RequireApproval` the enforcer produced. `verifyResult` is stated as structural verification only (`exitCode == 0 && !timedOut && target == request.targetType`) — it cannot and does not verify a command's actual semantic effect. `routeExecution` never itself calls `target.execute` — deciding and executing stay separate, the same boundary `PolicyDecision`/CAP-008's `RequiresApproval` already keep. Unit-tested (`DefaultExecutionTargetRouterTest`, 12 cases, real non-mocked `SecurityPolicy`/`SecurityPolicyEnforcer`/`ToolRegistry` matching `SecurityPolicyEnforcerTest`/`SecureToolExecutorTest`'s own conventions): no-capable-target short-circuit (registry/enforcer never consulted), unregistered-toolId naming, a `Deny` overriding an otherwise-capable target, least-privilege selection across `PrivilegeLevel`s, deterministic id tie-break, `Allow`/`RequireApproval` both attached correctly, the target itself never invoked by routing, and all four `verifyResult` cases (clean success, non-zero exit, timeout, target mismatch). |
| core-agent: CapabilityState / CapabilityMetadata / CapabilityFilter / CapabilityManager / CapabilityHealthChecker / DefaultCapabilityManager (CAP-009, first slice) | IMPLEMENTED (structural registry + real `CapabilityRegistry` integration; no real health checker for any provider) | Added 2026-09-11 — `CapabilityManager.kt`, keeping the promise CAP-008's own doc comment made ("a real extension point for CAP-009 to fill later"). **A fourth naming collision this session has hit, resolved the same way as the third:** P1.1's own section defines `interface CapabilityRegistry` with a completely different shape (`getCapability`/`listCapabilities`/`register`/`reverify`/`invalidate`) than the `isRegistered(id): Boolean` fun interface CAP-008 already shipped under that exact name. The new interface is named `CapabilityManager` instead — P1.1's own section title ("Capability Manager (with re-verification)") — avoiding the collision without inventing an unrelated name. `CapabilityState` (11 values) and `CapabilityMetadata` are shipped verbatim, reusing `CapabilityId`/`RiskTier` from CAP-008. `CapabilityFilter` is designed here (referenced by the spec as a default constructor arg, never defined — the same "referenced but never defined" gap `AllocatedContext`/`ProviderPreferences` already hit), kept to the two fields `CapabilityMetadata` itself makes filterable. **The real payoff, not just a rename:** `DefaultCapabilityManager` implements *both* `CapabilityManager` and CAP-008's `core-agent.CapabilityRegistry` — `isRegistered(id)` is `getCapability(id)?.state in {AVAILABLE, ENABLED}`, so `DefaultExecutionRouter` can now be constructed directly against a real `DefaultCapabilityManager`, proven by three integration tests (unregistered/`DISABLED`/`AVAILABLE`) exercising the real CAP-008 class, not a re-implemented fake. **Re-verification logic, made concrete without fabricating async infrastructure this codebase doesn't have:** the spec's own "kick off an async re-verify, return current state immediately" is honestly unbuildable with this repository's only async primitive (`Scheduler`, recurring-only, unsuited to a one-off background job writing back into a shared map) — so staleness became a synchronous, explicit query (`isStale`) instead of an invisible background action; `getCapability`/`listCapabilities` never silently reverify (pure lookups, matching `KnowledgeStore.load`/`ToolRegistry.get`), and a caller checks `isStale` then explicitly calls `reverify` — the same "decision, not automatic action" boundary every `PolicyDecision`/`RoutingDecision`/`RequiresApproval` shape already keeps this session. `register` overwrites (an upsert, like `KnowledgeStore.save`, not `ToolRegistry.register`'s duplicate-rejection, since capability metadata is expected to evolve). `reverify` throws `UnknownCapabilityException` for an id never registered (the signature is non-nullable, unlike `getCapability`); `invalidate` clears `lastVerifiedAt` for a registered id and no-ops for an absent one. `suspend` dropped from every method, the same zero-coroutines-dependency deviation stated repeatedly this session. Unit-tested (`CapabilityManagerTest`, 20 cases): register/overwrite/absence, filter by state/provider/both, `reverify` persisting a real state transition and throwing for an unknown id without ever invoking the health checker, `invalidate` clearing vs. no-op, all four `isStale` cases, `isRegistered`'s state-based true/false split, and the three real `DefaultExecutionRouter` integration cases. |
| core-agent: PermissionCategory / PrivilegeEscalationTier / leastEscalatedAvailable (CAP-010, first slice) | IMPLEMENTED (the CRITICAL root-equivalence rule is real and enforced; the rest of the taxonomy is declared, nothing else to gate yet) | Added 2026-09-11 — `PermissionPolicy.kt`. P1.2 is unusually thin — no Kotlin types at all, just a 12-value permission taxonomy, one CRITICAL rule, and a 7-tier escalation-preference list. **Deliberate design call: `SecurityLevel` (NORMAL/SENSITIVE/ROOT) is not touched, renamed, or replaced** — it's a real, tested, working risk-*level* axis with real callers across this repository; the spec's 12-item list is a permission-*domain* taxonomy, a different question, so this is an additive extension (a new `ToolSpec.requiredPermissionCategories: Set<PermissionCategory> = emptySet()` field, confirmed via grep to be non-breaking against all ~96 existing `ToolSpec(...)` call sites, all of which use named arguments) rather than a rewrite of a shipped contract. `PermissionCategory` (12 values verbatim) carries the CRITICAL warning directly on its own `CONTAINER` entry's doc comment ("root-equivalent — never presented as a peer permission to VIEW/AUTOMATION"), not only at class level. `PrivilegeEscalationTier` (7 values) encodes the roadmap's escalation preference order via declaration order — the same discipline `ContextKind` already documents for itself — with the spec's own caveats ("Termux is not a privilege tier"; "Shizuku ... is the API surface, not the privilege grant") carried verbatim on their own entries rather than silently resolved. `leastEscalatedAvailable(tiers): PrivilegeEscalationTier?` is real, if small, logic over that ordering (lowest ordinal wins, `null` for an empty set) rather than a bare enum nothing touches. **Not built in this slice:** no reconciliation of `SecurityLevel`/`RiskTier`/`CapabilityState`/`core-root.PrivilegeLevel` into one vocabulary (each stays its own axis, same "not unified here" call CAP-008/009/011 already made toward each other); no consumer of `PrivilegeEscalationTier`/`leastEscalatedAvailable` anywhere yet (a real, tested, standalone extension point, same relationship `ContextKind`/`CapabilityRegistry` had before later slices filled them in); no gating tied to `VIRTUALIZATION`/`INFRASTRUCTURE` (no real provider for either exists in this repository). Unit-tested (`PermissionPolicyTest`, 5 cases): `PrivilegeEscalationTier.entries`' literal order matches the spec's list exactly, and `leastEscalatedAvailable` across empty/single/mixed inputs, order-independent of iteration order. See the `SecurityPolicyEnforcer` row below for the CRITICAL rule's actual enforcement. |
| core-security: SecurityPolicyEnforcer root-equivalent permission-category gate (CAP-010, extends the existing row above) | IMPLEMENTED | Added 2026-09-11 — `SecurityPolicyEnforcer.authorize`'s existing `requiresRoot` gate widened (not replaced) to also fire when `ToolSpec.requiredPermissionCategories` contains `PermissionCategory.ROOT` or `PermissionCategory.CONTAINER`: `requiresRootEquivalent = spec.requiresRoot || spec.requiredPermissionCategories.any { it == ROOT \|\| it == CONTAINER }`. This is the concrete, testable implementation of P1.2's own CRITICAL rule — a `CONTAINER`-scoped tool is denied under exactly the same conditions (`rootEnabled`/`rootAvailable()`) a `requiresRoot = true` tool already is, never a lesser, peer-level check. Every other `PermissionCategory` triggers no special gate — the spec names only `CONTAINER` (and, by direct naming, `ROOT`) as root-equivalent; `VIRTUALIZATION`/`INFRASTRUCTURE` are separate, later roadmap items with no real provider in this repository to gate at all yet, so no policy is invented for them. Existing behavior for every `ToolSpec` left at the new field's default `emptySet()` is provably unchanged (`any {}` over an empty set is always `false`) — confirmed by the full, unmodified pre-existing `SecurityPolicyEnforcerTest` suite passing unchanged. Unit-tested (5 new cases appended to `SecurityPolicyEnforcerTest`, existing 9 cases untouched): a `CONTAINER`-category tool denied when root is disabled, and when root is enabled but unavailable; a `ROOT`-category tool denied the same way; non-root-equivalent categories (`VIEW`/`AUTOMATION`) never triggering the gate even with root disabled; a `CONTAINER`-category tool passing the gate once root is enabled and available. |
