...
| core-agent: objective analysis stage (ROADMAP-064) | IMPLEMENTED (interface + wiring), no concrete analyzer ships in this environment | Added 2026-09-09 — `ObjectiveAnalyzer.kt` (`ObjectiveAnalysis` data class: requirements/constraints/dependencies/tasks/verificationCriteria; `ObjectiveAnalyzer` functional interface). `ObjectiveEngine` gained an optional `analyzer` constructor param (default `null`, so every existing caller is unaffected); when set, its `ObjectiveAnalysis` is computed once before the first planning iteration and appended to the conversation context, so the planner's actual input changes — a real stage, not just a conceptual one. Split from a concrete implementation the same way `Planner`/`LlmPlanner` and `Logger`/`ConsoleLogger` already are: decomposing an objective's text needs an LLM call in practice, which `core-agent` has no dependency on — a concrete analyzer belongs in a module like `core-llm` |
...
| core-agent: dependency-ordered task execution (ROADMAP-067) | IMPLEMENTED (new infrastructure), not yet wired into ObjectiveEngine's main loop | Added 2026-09-09 — `TaskGraph.kt` (`TaskSpec`: a named tool invocation plus a `dependsOn` set; `TaskGraph` validates eagerly at construction — unique names, every dependency resolvable — and computes `executionWaves()`, a topological grouping, throwing `CyclicTaskGraphException` rather than returning a partial order) and `DependencyOrderedExecutor.kt` (runs a `TaskGraph` wave by wave through `ToolExecutor`; a task whose dependency `Failure`d — or was itself skipped, so a broken chain propagates transitively — is skipped with the reason recorded, never silently omitted; `Partial`/`Unexpected` dependencies don't block, matching `ToolExecutor`'s own only-`Failure`-blocks distinction). Self-contained: neither `ObjectiveEngine` nor `Planner` constructs or consumes a `TaskGraph` yet — wiring a planner that can emit a multi-task plan into this executor remains a follow-up |
