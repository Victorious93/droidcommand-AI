...
| core-agent: structured logging (ROADMAP-014) | IMPLEMENTED (in ObjectiveEngine and ToolExecutor) | Added 2026-09-09 — `Logger.kt` (`Logger` interface, `NoOpLogger` default, `ConsoleLogger` real implementation); `ObjectiveEngine` gained an optional `logger` constructor param (default `NoOpLogger`, so every existing caller is unaffected) logging `objective_started`/`objective_completed`/`objective_aborted`/`objective_cancelled`/`objective_exhausted_iterations`/`tool_result`/`unknown_tool` events. `ToolExecutor` — the executor Pilot Mode invokes directly, as well as the one `ObjectiveEngine` drives for Forge Mode — gained the same optional `logger` param, logging `tool_result`/`tool_mode_rejected`/`tool_initiator_rejected`/`tool_cancelled`, so Pilot Mode now has the same operational visibility Forge Mode already had. `core-security`'s `SecureToolExecutor` is still not wired to a logger — a follow-up, not silently assumed covered |
