...
│                            PLANNED — gated on an Android SDK/device this
│                            environment does not have.
│
├── cli                      Pure Kotlin/JVM device-free CLI entrypoint
│                            (added 2026-09-12). Dispatches Pilot
│                            instructions and Forge objectives against a
│                            real DroidCommandSession, using
│                            LlmProviderFactory.createPlanner for its LLM
│                            planner — the "or a future CLI" half of the
│                            entrypoint gap `app` above names, needing
│                            neither an Android SDK nor a device.
│
...
Both modes route through the same `ToolRegistry` and `ToolRunner` (the plain
`ToolExecutor` by default, or `core-security.SecureToolExecutor` when a
caller needs policy/approval/grant/audit gating — see `ToolRunner`'s
ARCHITECTURE.md row) so a tool is written once and works in either mode. Pilot Mode invokes a single tool per
...
planner, and either runs a tool through its `ToolRunner` or terminates. Agent
...
| core-agent: ToolExecutor + bounded retry | IMPLEMENTED | `ToolExecutor.kt`, compiles, unit-tested. As of 2026-09-12, implements the new `ToolRunner` interface (see its own row below); `run`'s override signature gained a trailing `grantId: String?` parameter it accepts and ignores, purely to satisfy that interface — every existing call site is unaffected since it's optional and trailing |
...
| core-agent: ToolRunner (shared executor contract) | IMPLEMENTED | Added 2026-09-12 — `ToolRunner.kt`: the minimal `run(toolName, input, retryPolicy, isCancelled, mode, initiator, grantId): ToolResult` interface both `ToolExecutor` and `core-security.SecureToolExecutor` implement, letting `DroidCommandSession`/`ObjectiveEngine` hold either behind one field (`executor: ToolRunner`) without knowing which. This closes the gap the `cli` module's 2026-09-12 entry named: those two classes previously hard-typed `executor: ToolExecutor`, so a caller could never substitute the policy/approval/grant/audit-gated `SecureToolExecutor` in — meaning `SENSITIVE`/`ROOT` tools could never be safely registered against a real `DroidCommandSession`. **A real latent bug found and fixed in the same slice:** `SecureToolExecutor.run` already delegated its actual invocation to a real `ToolExecutor.run(...)`, but only forwarded 4 of its 6 parameters, silently omitting `mode`/`initiator` — since `ToolExecutor`'s own `allowedModes` check is skipped whenever `mode == null` (its default), `SecureToolExecutor` never enforced `ToolSpec.allowedModes` at all. Fixed by (a) forwarding `mode`/`initiator` through to the delegate call, and (b) adding an earlier, explicit `mode`-not-allowed check (new `AuditEventType.MODE_DENIED`) ahead of any approval prompt/grant check/audit record — a tool not offered in the requested mode is now denied outright, the same "hard deny before any side effect" ordering the existing `requiredInitiator` check already used. `SecureToolExecutor.initiator` is now nullable (`Initiator? = null`, matching the interface); a `null` value is treated as `Initiator.AI` internally, preserving every existing test's default behavior. Not built in this slice, named rather than silently skipped: `cli/Main.kt` still doesn't depend on `core-security` or construct a `SecureToolExecutor`, and no `SENSITIVE`/`ROOT` tool is registered in the CLI yet — both remain the next follow-up. Unit-tested: 4 new `SecureToolExecutorTest` cases (mode-denied-without-prompting, mode-allowed, null-mode-unaffected, `MODE_DENIED` log assertion) plus a new `DroidCommandSessionSecureToolExecutorIntegrationTest` (4 cases) proving a real `DroidCommandSession` driven entirely by a real `SecureToolExecutor` through both Pilot and Forge mode, including the mode-check fix |
| core-agent: ObjectiveEngine (bounded Forge loop) | IMPLEMENTED | `ObjectiveEngine.kt`, compiles, unit-tested incl. the maxIterations bound. As of 2026-09-05, a planner naming an unregistered/wrong-mode tool no longer fails the objective outright — it's told what's actually available and replans, still bounded by maxIterations. As of 2026-09-12, `executor` is typed as `ToolRunner`, not the concrete `ToolExecutor` — see that row for why |
...
| core-agent: DroidCommandSession (Pilot/Forge mode switching) | IMPLEMENTED | `DroidCommandSession.kt`, unit-tested incl. a rejected mode switch attempted mid-task. As of 2026-09-12, `executor` is typed as `ToolRunner`, not the concrete `ToolExecutor` — see that row for why |
...
| app (Android shell) | PLANNED | No directory, Gradle file, or manifest exists yet — nothing scaffolded, and no Android SDK in this environment either (Section 7). Corrected 2026-09-05: an earlier version of this row implied a manifest/Gradle scaffold already existed on disk; it does not — see `docs/AUDIT_2026-09-05.md`. **Distinct from the new `cli` module below (2026-09-12):** that one is the separate device-free entrypoint every prior addendum named alongside this one ("`app` module or a future CLI") — it does not replace or scaffold this row, which stays PLANNED and gated on an Android SDK/device exactly as before. |
...
| core-security: SecureToolExecutor (controlled execution boundary) | IMPLEMENTED | `SecureToolExecutor.kt`, unit-tested incl. "denied tool is never invoked" and "AwaitingApproval before prompting". As of 2026-09-05, also enforces an optional grant lifecycle and audit-log fail-closed behavior (both additive, default off). As of 2026-09-12, implements `core-agent.ToolRunner` (so it can stand in for `ToolExecutor` inside `DroidCommandSession`/`ObjectiveEngine`) and enforces `ToolSpec.allowedModes` for real — see `core-agent`'s `ToolRunner` row for the full record of the bug this closed and the fix |
...
| core-llm-factory: LlmProviderFactory (provider-construction factory, config-driven multi-provider construction, construction half) | IMPLEMENTED | Added 2026-09-12, per the same-day scoping addendum's design, built without deviation: `LlmProviderFactory.kt` (module `core-llm-factory`, depending on `core-agent`/`core-llm`/`core-config`/`core-remote`/`core-llm-anthropic`/`core-llm-openai`). `build`/`buildAll`/`load` switch on `ConfiguredLlmProvider.config.provider` case-insensitively to construct a real `AnthropicLlmProvider`/`OpenAiLlmProvider` over a caller-supplied, shared `HttpTransport`, pairing each with its own `AiProviderInfo` into a `RegisteredProvider`; a new `UnknownLlmProviderException` names the bad value for anything else, and `buildAll` fails the whole call on one bad entry rather than returning a partial list. This closes the provider-construction-factory half named across the CAP-004/005/006/007 addenda and `MultiLlmConfigLoader`'s own doc comment — `core-config.MultiLlmConfigLoader`/`ConfiguredLlmProvider` (config-loading half, already IMPLEMENTED above) plus this module now together take a `ConfigSource` all the way to a real, running `List<RegisteredProvider>` in one `LlmProviderFactory.load` call. **Same-day follow-up (2026-09-12, "wire LlmProviderFactory into AiProviderSelector/ModelRouter"):** added `createSelector(source, transport, tokenBudgetManager = null): AiProviderSelector` (`load`s then wraps in a real `DefaultAiProviderSelector`) and `createModelRouter(source, transport): ModelRouter` (`load`s then wraps in `ModelRouter(LocalFirstOrdering.order(...))`, the exact composition `LocalFirstOrdering`'s own doc comment already names) — this module now composes with `core-llm`'s own selection/routing layer directly from a `ConfigSource`, closing the "reachable but not yet invoked anywhere else" gap this row named a same-day entry earlier. **Not built, named rather than silently assumed:** no `fun main()`/CLI/app entrypoint (still the PLANNED `app` module's job, or a future device-free CLI); no caching/pooling/singleton lifecycle for constructed providers; no third provider implementation beyond `"anthropic"`/`"openai"`; `createModelRouter`'s only "failure mode" for an unconfigured source is `ModelRouter`'s own pre-existing `require(providers.isNotEmpty())`, not a new bespoke check — an empty `ConfigSource` surfaces as `IllegalArgumentException`, not a silently-empty router. Unit-tested (`LlmProviderFactoryTest`, 14 cases, no real network — an inline fake `HttpTransport` never actually receives a call in any case): the original 10 (construction/`build`/`buildAll`/`load` cases, see below) plus 4 new — `createSelector` exposing every loaded provider's `AiProviderInfo` via `listProviders()`; `createSelector` over an unconfigured source having nothing to select (`selectProvider` returns `null`); `createModelRouter` ordering providers local-first regardless of declaration order (a `CLOUD`-typed provider declared first, a `LOCAL`-typed one second — proven via `router.config.provider` matching the local provider's own `config.provider`, since `ModelRouter.config` is always its first-tried provider's config); `createModelRouter` over an unconfigured source throwing `IllegalArgumentException` rather than building an empty router. Original 10 cases: `"anthropic"`/`"openai"` building the correct concrete provider; case-insensitive matching; an unknown provider throwing `UnknownLlmProviderException` naming the bad value; `build` pairing the real provider with the exact same `info`/`config` instances passed in; `buildAll` preserving order and producing one `RegisteredProvider` per entry; a single unknown provider among valid ones failing the whole `buildAll` call; `buildAll` constructing distinct provider instances over one shared transport without error; `load` round-tripping a `MapConfigSource` through `MultiLlmConfigLoader` into real constructed providers, and returning an empty list when nothing is configured. **Same-day follow-up (2026-09-12, "wire it into ObjectiveEngine/DroidCommandSession next"):** added `createPlanner(source, transport): Planner` — `createModelRouter`s `source` and wraps the result in a real `ai.droidcommand.llm.LlmPlanner`, producing the exact `ai.droidcommand.agent.Planner` `core-agent`'s `ObjectiveEngine`/`DroidCommandSession.runForgeObjective` take as a parameter. No change to `core-agent` itself, and none was needed or made: `core-agent` depends on none of `core-llm`/`core-config`/`core-remote`/`core-llm-anthropic`/`core-llm-openai` and must not gain such a dependency — `Planner` was already the seam that lets a caller hand either of them a real LLM-backed planner without `core-agent` ever knowing a concrete provider exists (`core-llm.ObjectiveEngineIntegrationTest` already proved this composition by hand with a scripted `LlmProvider`); `createPlanner` is that same composition's other end, reachable directly from a `ConfigSource`. Integration-tested (`LlmProviderFactoryPlannerIntegrationTest`, 2 cases, real request-encoding/response-parsing through the real `AnthropicLlmProvider` construction this module already does — only the socket itself is scripted, at the HTTP-transport layer, not the `LlmProvider` layer, matching `AnthropicLlmProviderIntegrationTest`'s own real-JSON-not-real-network precedent): a full Forge Mode objective (tool call, then completion) run end to end through a real `DroidCommandSession.runForgeObjective` call using a planner built entirely by `createPlanner`, reaching `AgentState.Completed`; a scripted HTTP 401 response correctly classified by the real `AnthropicLlmProvider` into `LlmError.Authentication`, then `PlannerDecision.Abort`, then `AgentState.Failed` — proving the failure path survives the full real stack, not just the scripted-provider version `ObjectiveEngineIntegrationTest` already covered. **Still not built, unchanged:** nothing constructs a `createPlanner`/`createSelector`/`createModelRouter` call from any actual running entrypoint (`app` module, a CLI) — this row's own composition is complete and tested, but nothing in this repository invokes it outside test code, since no such entrypoint exists yet. |
| cli: device-free CLI entrypoint (`Main.kt`/`EchoTool.kt`) | IMPLEMENTED (first slice; see deviation notes) | Added 2026-09-12, closing the "not built, named rather than silently assumed" gap the `core-llm-factory` row above ends on: a new module `cli` (depending on `core-agent`/`core-config`/`core-remote`/`core-llm-factory`), the `application` Gradle plugin, `fun main(args)` dispatching three commands — `list-tools`, `pilot <tool> [key=value ...]` (calls `DroidCommandSession.runPilotInstruction`), and `forge <objective text>` (switches the session to `AgentMode.FORGE`, builds a `Planner` via `LlmProviderFactory.createPlanner(EnvConfigSource(), JdkHttpTransport())`, then calls `runForgeObjective`). The `Planner` is built lazily, only inside `forge`, so `pilot`/`list-tools` need no LLM configuration at all. Every constructor/factory call is real, matching `LlmProviderFactoryPlannerIntegrationTest`'s own established assembly exactly (same `ToolRegistry`/`AgentStateMachine`/`ToolExecutor`/`DroidCommandSession` shape) — nothing in this module is mocked. Both `pilot`/`forge` catch every exception the real stack can throw (`MissingConfigException`/`InvalidLlmProviderConfigException`/`UnknownLlmProviderException`/`IllegalArgumentException`/`UnknownToolException`/a bad `key=value` pair) and print a one-line explanation plus a non-zero exit code, never a raw stack trace — proven directly by `MainTest`'s "without any configured LLM provider" and "against an unregistered tool" cases. **A deliberate, stated scope limit, not an oversight:** this first slice registers only one tool, a harmless side-effect-free `EchoTool` (`NORMAL` security level) — it does **not** wire `core-shell.ShellTool`/`core-root.RootTool`/`core-build.BuildTool`/any other `SENSITIVE`/`ROOT`-level tool into this CLI. Investigated directly before writing anything: every one of those is designed to run behind `core-security.SecureToolExecutor`'s policy/approval/audit gate (see `ShellTool`'s own doc comment — "gated ... exactly like `core-build.BuildTool`"), but `DroidCommandSession`/`ObjectiveEngine` both take a concrete `ai.droidcommand.agent.ToolExecutor`, not an interface `SecureToolExecutor` could substitute for — confirmed by reading both constructors directly, not assumed. No file anywhere in this repository currently composes `SecureToolExecutor` with `DroidCommandSession`; every existing `*SecureExecutorIntegrationTest` calls `SecureToolExecutor.run` directly instead. Wiring a confirmation-requiring tool straight into this CLI's plain, unguarded executor would silently bypass the approval/audit layer this codebase built for exactly that purpose — for Forge mode, an LLM-driven planner deciding on its own to run an allow-listed shell command with no human confirmation and no audit record — so this slice deliberately does not do that instead of building an unreviewed capability-escalation path. **Real, named follow-up work, not built here:** (1) either widening `DroidCommandSession`/`ObjectiveEngine` to accept a `SecureToolExecutor`-compatible executor (a `core-agent` change, needs its own scoping pass) or composing `SecureToolExecutor` in front of this CLI's dispatch by hand, whichever a future session scopes; only then should `ShellTool`/`RootTool`/etc. be registered here; (2) no interactive/REPL mode — one command per process invocation only; (3) no persistent `ConversationStore`/`KnowledgeStore` wiring between invocations, so each `forge` call starts a fresh `ConversationContext`; (4) no `--config <file>` flag — only `EnvConfigSource` (process environment), matching `LlmProviderFactory.createPlanner`'s own `ConfigSource` parameter with no CLI-specific config format invented. Unit-tested (`MainTest`, 9 cases, mirroring `LlmProviderFactoryPlannerIntegrationTest`'s own `MapConfigSource`/scripted-`HttpTransport` fixtures for the `forge` cases rather than touching real environment variables or the network): the echo tool being registered; `pilot`/`forge` each failing cleanly with no arguments; a successful `pilot echo` round-trip; an invalid `key=value` pair and an unregistered tool name each failing cleanly rather than throwing; `forge` failing cleanly with no LLM provider configured; a full Forge objective (tool call, then completion) driven end to end through a real `DroidCommandSession.runForgeObjective` call using a planner built entirely by `LlmProviderFactory.createPlanner` over a scripted HTTP transport, reaching `AgentState.Completed`; and a scripted HTTP 401 surfacing as a failed run (non-zero exit), not a crash. Manually smoke-tested via `./gradlew :cli:run --args="..."` for `list-tools`, `pilot echo text=...`, `help`, and `forge` with no LLM configured (clean error, no stack trace) — real process invocations, not just the unit tests above. |
