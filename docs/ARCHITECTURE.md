...
| core-llm: ProviderLocality / LocatedProvider / LocalFirstOrdering (CAP-003, first slice) | IMPLEMENTED | Added 2026-09-11 — `ProviderLocality.kt`, closing the "prefer local before remote AI" gap the CAP-002 addendum named. `ModelRouter` already composed an ordered `List<LlmProvider>` with fallback-on-transient-failure but nothing built that order; this adds a two-value `ProviderLocality` (`LOCAL`/`REMOTE` — deliberately narrower than the roadmap prompt's separate, four-way `ProviderType` sketch for the larger CAP-004 `AiProviderSelector`, not attempted here), `LocatedProvider` (pairs an `LlmProvider` with a caller-declared locality — never inferred from `LlmConfig.endpoint`, since a hostname is not a reliable locality signal), and `LocalFirstOrdering.order(providers): List<LlmProvider>`, a pure stable sort (local-first, ties broken by input order) producing exactly the list `ModelRouter`'s constructor already accepts. `ModelRouter`/`LlmPlanner` are both untouched — zero changes to any existing file. **Not built in this slice, named rather than silently skipped:** config-driven multi-provider construction (`core-config` is still strictly single-provider; no `List<LlmConfig>` shape exists), endpoint-based locality inference (rejected as unreliable, not merely deferred), and CAP-004's richer capability/latency/cost/privacy-aware selection. Unit-tested (`ProviderLocalityTest`, scripted `LlmProvider` fakes matching `ModelRouterTest`'s convention, no mocking): all-local and all-remote inputs preserve relative order, a mixed input puts every local provider before every remote one preserving relative order within each tier, empty input, determinism across repeated calls, and an integration-style case feeding the ordered list into a real `ModelRouter` proving the local provider is tried first and the remote one is used as fallback. |
