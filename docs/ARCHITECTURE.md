...
| core-root: `RootExecutionTarget` (CAP-011, second real ExecutionTarget implementation) | IMPLEMENTED (see deviation notes) | Added 2026-09-12 — `RootExecutionTarget.kt`, the second real `core-security.ExecutionTarget` in this repository (`core-shell.LocalProcessExecutionTarget` was the first) — wraps any `RootExecutor` (typically `MagiskProvider`), giving `DefaultExecutionRouter` (CAP-012) a genuinely different second target type (`ExecutionTargetType.ANDROID`, versus `LocalProcessExecutionTarget`'s `LOCAL_PC`) and privilege level to route between, rather than only ever exercising its least-privilege selection against scripted fakes. `isHealthy()` delegates to `RootExecutor.isRootAvailable()` — genuinely checks, unlike `LocalProcessExecutionTarget`'s unconditional `true` (a local process has no external liveness dependency; root does). **Honest, explicit limitation, not a silent behavior change:** `RootCommand` has no `workingDirectory`/`environment` fields at all (unlike `core-shell.ShellCommand`), so this target cannot honor a caller-supplied `workingDir`/`env` — rather than silently ignoring them, `execute` fails explicitly (without ever invoking the underlying `RootExecutor`) naming exactly which parameter isn't supported and why. `timedOut` is derived the same way `LocalProcessExecutionTarget`'s already is: matching `RootExecutionResult.Failure.reason`'s literal `"Command timed out"` prefix, since `MagiskProvider.execute` reports a timeout via that same string pattern (confirmed by reading `MagiskProvider.kt` directly) and `RootExecutionResult.Failure` carries no distinct timeout field either. `verified` stays honestly `false`. **Not built in this slice:** widening `RootCommand` to carry working-directory/environment (would ripple through every `RootExecutor` implementation and test); a cross-module integration test exercising `LocalProcessExecutionTarget` and `RootExecutionTarget` together through one `DefaultExecutionRouter` — `core-shell` and `core-root` are siblings with no dependency on each other, and no third module currently depends on both, so this specific two-real-targets scenario is covered by `DefaultExecutionRouterTest`'s scripted-fake least-privilege cases instead, honestly noted as a structural gap rather than forced. Unit-tested (`RootExecutionTargetTest`, 10 cases): `ANDROID` type; `isHealthy()` reflecting the executor's real `isRootAvailable()` in both directions; id/context/capabilities pass-through; empty-argv/non-null-workingDir/non-null-env all failing without calling the executor; argv splitting and timeout forwarding; `Success`/non-timeout-`Failure`/timeout-`Failure` mapping. |
...
| core-security: PermissionCategory / ROOT_EQUIVALENT_CATEGORIES / SecurityPolicy.isCategoryGranted / EscalationTier (CAP-010, Policy & Permission Engine extension, first slice) | IMPLEMENTED (see deviation notes) | Added 2026-09-12 — `PermissionCategory.kt` plus one new optional constructor parameter on `SecurityPolicy`. `PermissionCategory`'s 13 values are verbatim from the roadmap prompt's P1.2 list (an earlier CAP-010 audit row above miscounted this as 12; corrected here rather than perpetuated). **Deliberate deviation from a literal reading of the prompt's own "extend the existing `SecurityLevel` enum" heading:** `SecurityLevel` itself is unchanged — `PermissionCategory` is a new, separate type. Folding 13 category values into the existing 3-value `SecurityLevel` (a confirmation-requirement axis every `ToolSpec`/`SecurityPolicyEnforcer.authorize` call site already depends on) would conflate two orthogonal concepts, the same "distinct axis, not a competing model" relationship `RiskTier`/`SecurityLevel` already have. The prompt's own "CRITICAL: Docker/container socket access is root-equivalent... Document this explicitly in the policy engine" instruction is made an executable gate, not just a comment: `ROOT_EQUIVALENT_CATEGORIES = {ROOT, CONTAINER}`, consulted by the new `SecurityPolicy.isCategoryGranted(category)` extension function, which fails closed — `CONTAINER` (or `ROOT`) listed in the new `SecurityPolicy.grantedCategories` field is still denied unless `rootEnabled` and a true `rootAvailable()` both hold. `EscalationTier` (7 values, declaration order = the prompt's own preference order, lowest-privilege first) captures the escalation sequence; `TERMUX`'s "not a privilege tier, a separate execution environment" and `SHIZUKU`'s "requires ADB or root to bootstrap, it's the API surface not the grant" caveats from the prompt's own "Note" are preserved as doc comments rather than silently dropped, since a bare enum value can't express them. **Additive throughout:** `SecurityPolicy.grantedCategories` defaults to `emptySet()`, and neither it nor `isCategoryGranted` is consulted by `SecurityPolicyEnforcer.authorize` or any existing `Tool`/`ToolSpec` — every existing `SecurityPolicy`/`SecurityPolicyEnforcer` caller and test is unaffected. **Not built in this slice:** no `PermissionCategory` field on `ToolSpec` (would ripple through every existing tool definition/test across the codebase); no wiring of `EscalationTier` into any real escalation logic (P2's Shizuku/Termux/ADB targets don't exist yet). Unit-tested (`PermissionCategoryTest`, 3 cases; `SecurityPolicyIsCategoryGrantedTest`, 7 cases; `EscalationTierTest`, 3 cases): category count and root-equivalent-set correctness; granted/not-granted for an ordinary category; `CONTAINER`/`ROOT` denied when root disabled, denied when root enabled-but-unavailable, granted only when both the root gate and `grantedCategories` membership hold, and denied when the root gate holds but membership doesn't; escalation tier ordering and the `TERMUX`-before-`SHIZUKU`/`ROOT`-last invariants. |
